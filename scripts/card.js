...
export function createCard(arrayOfObj) {
    ...
    for (let i = 0; i < arrayOfObj.length; i += 1) {
        ...
        if (getIndexInFavorite(arrayOfObj[i]) !== -1) {
            star.children[0].classList.add("active");
        }
        ...
        book.addEventListener('click', function (e) {
            if (e.target.closest('.book__fav')) {
                favorites(arrayOfObj[i], book);
            }
            else {
...
function getIndexInFavorite(bookObj) {
    let favorite = JSON.parse(localStorage.getItem("favorite")) || [];
    return favorite.findIndex((item) => item.bookID == bookObj.bookID);
}

function favorites(bookObj, book) {
    let favorite = JSON.parse(localStorage.getItem("favorite")) || [];
    let star = book.children[1].children[0].children[0];
    let index = getIndexInFavorite(bookObj);
    if (index !== -1) {
